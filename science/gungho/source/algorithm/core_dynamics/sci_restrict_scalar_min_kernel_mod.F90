...
!> @brief The min restriction operation for scalar fields.
!> @details Restrict a scalar field on a fine grid to a coarse grid field by taking min over coarse cell area.
!!          This kernel only works for the lowest-order W3 and Wtheta spaces

module sci_restrict_scalar_min_kernel_mod
...
type, public, extends(kernel_type) :: restrict_scalar_min_kernel_type
   private
...
end type restrict_scalar_min_kernel_type

public :: restrict_scalar_min_kernel_code

  ! Generic interface for real32 and real64 types
  interface restrict_scalar_min_kernel_code
    module procedure  &
      restrict_scalar_min_code_r_single, &
      restrict_scalar_min_code_r_double
  end interface

contains

  !> @brief Thein restriction operation for scalar fields.
  ...
  ! R_SINGLE PRECISION
  ! ==================
  subroutine restrict_scalar_min_code_r_single(              &
                                           nlayers,                 &
                                           cell_map,                &
    ...
    ! Assign a large positive number for the min function to act on
    coarse_value(:) = huge(0.0_r_single)
    ...
    do k = 0, top_df
      do y_idx = 1, ncell_fine_per_coarse_y
        do x_idx = 1, ncell_fine_per_coarse_x
          coarse_value(k+1) =  min(coarse_value(k+1), &
  ...
  end subroutine restrict_scalar_min_code_r_single

  ! R_DOUBLE PRECISION
  ! ==================
  subroutine restrict_scalar_min_code_r_double(              &
                                           nlayers,                 &
                                           cell_map,                &
                                           ncell_fine_per_coarse_x, &
                                           ncell_fine_per_coarse_y, &
    ...
    top_df = nlayers - 2 + ndf
    ! Assign a large positive number for the min function to act on
    coarse_value(:) = huge(0.0_r_double)
  ...
  end subroutine restrict_scalar_min_code_r_double


end module sci_restrict_scalar_min_kernel_mod
